...
- Como epidemiologistas, estatísticos, sociólogos, etc., e de fato como seres humanos, é algo sobre o qual __sabemos muito__.
...
- $r_C$ e $r_T$ representam as __respostas potenciais__.
...
- Uma destas respostas é observada: se $Z = 0$, $r_C$ é observada; se $Z = 1$, $r_T$ é observada (ou seja, $R = Z\times r_T + (1 - Z)\times r_C$).
...
- $\delta_i = r_{T_i} - r_{C_i} =\ ?$, para todo indivíduo $i$, pois uma das respostas potenciais nunca é observada.
...
- É por isso que __experimentos aleatórios__ são o __padrão-ouro__ para inferir efeitos causais.
...
- De maneira mais geral, se __existe__ um efeito causal de $Z$ em $R$, mas também uma associação não-causal devido a $X$, então o efeito causal será estimado com viés, a menos que estratifiquemos em $X$.
